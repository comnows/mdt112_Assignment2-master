...
namespace mdt112_assignment2
{
    ...
    class Quiz{
        ...
        {
            List<Garbage> garbagelist = new List<Garbage>();
            for(int p=0;p<garbageList.Count;p++)
            {
                Size a = garbageList[p].GetSize();
                if (a == Size.SMALL)
                    garbagelist.Add(garbageList[p]);
            }
            for (int q = 0; q < garbageList.Count; q++)
            {
                Size a = garbageList[q].GetSize();
                if (a == Size.MEDIUM)
                    garbagelist.Add(garbageList[q]);
            }
            for (int r = 0; r < garbageList.Count; r++)
            {
                Size a = garbageList[r].GetSize();
                if (a == Size.LARGE)
                    garbagelist.Add(garbageList[r]);
            }
        ...
        public static List<Garbage> GetSellableGarbageList( List<Garbage> garbageList ){
            ...
            List<Garbage> garbagelist = new List<Garbage>();
            for(int n = 0;n<garbageList.Count;n++)
            {
                if (garbageList[n] is ISellable)
                    garbagelist.Add(garbageList[n]);
            }
        ...
        public static float ComputeMaximumSellPrice( List<Garbage> garbageList ){
            ...
            List<MaterialGarbage> garbagelist = new List<MaterialGarbage>();
            List<float> garbagelistprice = new List<float>();
            
            for (int m = 0; m < garbageList.Count; m++)
            {
                if (garbageList[m] is ISellable)
                {
                    if (garbageList[m] is IDisassemblable)
                    {
                        ElectronicsGarbage output = (ElectronicsGarbage)garbageList[m];
                        garbagelist = output.Disassemble();
                        float DisassembleCost = 0.0f;
                        float Price = 0.0f;
                        for(int y=0;y<garbagelist.Count;y++)
                        {
                            Price = garbagelist[y].GetSellPrice();
                            DisassembleCost = Price + DisassembleCost;
                        }
                        ...
                        {
                            float b = 1.2f;
                            float price = output.GetSellPrice() + DisassembleCost - (b * output.GetDisassembleCost());
                            garbagelistprice.Add(price);
                        }
                    ...
                    else if(garbageList[m] is GlassBottleGarbage)
                    {
                        GlassBottleGarbage output = (GlassBottleGarbage)garbageList[m];
                    ...
                    {
                        MaterialGarbage output = (MaterialGarbage)garbageList[m];
                        float price = output.GetSellPrice();
            ...
            float maxprice = 0.0f;
            foreach(float HighestPrice in garbagelistprice)
            {
